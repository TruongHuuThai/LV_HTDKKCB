...
export default function AppointmentDetailPage() {
  ...
  const [cancelReason, setCancelReason] = useState('B?nh nhân ch? ð?ng h?y');
  ...
  const handleCancel = () => {
    cancelMutation.mutate(
      { appointmentId, payload: { reason: cancelReason || 'B?nh nhân ch? ð?ng h?y', source: 'WEB' } },
      {
        onSuccess: (result) => {
          toast.success(result.message || 'Ð? h?y l?ch h?n.');
        },
        onError: (error) => {
  ...
  const handleReschedule = () => {
    if (!selectedSlot) {
      toast.error('Vui l?ng ch?n slot m?i trý?c khi ð?i l?ch.');
      return;
    }
    rescheduleMutation.mutate(
      ...
      {
        onSuccess: (result) => {
          toast.success(result.message || 'Ð?i l?ch thành công.');
        },
        onError: (error) => {
          logFrontendError('appointment-detail-reschedule', error, { appointmentId });
          toast.error(getPatientFlowErrorMessage(error, 'Không th? ð?i l?ch h?n này.'));
        },
      },
    );
  };
  ...
  if (detailQuery.isLoading) {
    return <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8 text-slate-600">Ðang t?i chi ti?t l?ch h?n...</div>;
  }
      ...
      <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
        <Card className="border-slate-200">
          <CardHeader>
            ...
            <CardDescription>
              Theo d?i tr?ng thái l?ch, thanh toán và các m?c thay ð?i quan tr?ng.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4 text-sm">
            <div className="grid gap-3 sm:grid-cols-2">
              <InfoRow label="Bác s?" value={appointment.LICH_BSK?.BAC_SI?.BS_HO_TEN || 'Chýa có d? li?u'} />
              <InfoRow label="Chuyên khoa" value={appointment.LICH_BSK?.BAC_SI?.CHUYEN_KHOA?.CK_TEN || 'Chýa có d? li?u'} />
              <InfoRow label="Ngày khám" value={String(appointment.N_NGAY || '').slice(0, 10)} />
              <InfoRow label="Bu?i" value={appointment.B_TEN || 'Chýa có d? li?u'} />
              ...
              <InfoRow label="Ph?ng" value={appointment.LICH_BSK?.PHONG?.P_TEN || 'Chýa có d? li?u'} />
            </div>

            <div className="rounded-2xl border border-slate-200 p-4">
              <p className="mb-2 font-medium text-slate-900">Thông tin ti?n khám</p>
              <p className="text-slate-700"><strong>Tri?u ch?ng:</strong> {detailQuery.data?.preVisit?.symptoms || 'Chýa c?p nh?t'}</p>
              <p className="mt-1 text-slate-700"><strong>Ghi chú:</strong> {detailQuery.data?.preVisit?.note || 'Chýa c?p nh?t'}</p>
            </div>

            <div className="rounded-2xl border border-slate-200 p-4">
              <p className="mb-2 font-medium text-slate-900">Timeline thông báo g?n nh?t</p>
              {(detailQuery.data?.notifications || []).length === 0 ? (
                <p className="text-slate-500">Chýa có thông báo g?n ðây.</p>
              ) : (
        ...
        <div className="space-y-6">
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
                {cancelPolicyQuery.data?.canCancel
                  ? `B?n có th? h?y l?ch trý?c ${cancelPolicyQuery.data?.cancelDeadlineAt || 'h?n policy'}.`
                ...
                {cancelPolicyQuery.data?.canCancel ? (
                  <>
                    ...
                    <Button variant="outline" onClick={handleCancel} disabled={cancelMutation.isPending}>
                      {cancelMutation.isPending ? 'Ðang h?y l?ch...' : 'H?y l?ch'}
                    </Button>
                  </>
                ) : null}
              </div>
          ...
          <Card className={`border-slate-200 ${action === 'reschedule' ? 'ring-2 ring-blue-200' : ''}`}>
            <CardHeader>
              <CardTitle>Ð?i l?ch khám</CardTitle>
              <CardDescription>
                Ch?n bác s?, ngày và slot m?i. H? th?ng s? ki?m tra tính h?p l? trý?c khi c?p nh?t.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              ...
              <Textarea
                placeholder="L? do ð?i l?ch (không b?t bu?c)"
              ...
              <Button onClick={handleReschedule} disabled={rescheduleMutation.isPending || !selectedSlot}>
                {rescheduleMutation.isPending ? 'Ðang ð?i l?ch...' : 'Xác nh?n ð?i l?ch'}
              </Button>
            </CardContent>
          </Card>
        </div>
